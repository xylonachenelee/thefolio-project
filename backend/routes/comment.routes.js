...
router.get('/:postId', async (req, res) => {
    try {
        const comments = await Comment.find({ post: req.params.postId })
            ...
            .sort({ createdAt: -1 }); // newest first (changed from 1 to -1)
        res.json(comments);
    } catch (err) { 
...
// POST /api/comments/:postId — User/Admin: add a comment
router.post('/:postId', protect, userOrAdmin, async (req, res) => {
    try {
        const comment = await Comment.create({
            post: req.params.postId,
            author: req.user._id,
            content: req.body.content, // Changed from 'body' to 'content'
        });
